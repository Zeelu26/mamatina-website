...
export async function GET() {
  ...
  return NextResponse.json({
    photos: [...db.heroPhotos].sort((a, b) => a.order - b.order),
  });
}
...
export async function POST(req: Request) {
  ...
  await updateDB((db) => {
    const nextOrder = (db.heroPhotos.reduce((m, p) => Math.max(m, p.order), 0) || 0) + 1;
    db.heroPhotos.push({
      id: uuid(),
      url,
  ...
  await updateDB((db) => {
    if (Array.isArray(body.order)) {
      body.order.forEach((id: string, idx: number) => {
        const p = db.heroPhotos.find((x) => x.id === id);
        if (p) p.order = idx + 1;
      });
    } else if (body.id) {
      const p = db.heroPhotos.find((x) => x.id === body.id);
      if (p) {
...
export async function DELETE(req: Request) {
  ...
  await updateDB((db) => {
    const idx = db.heroPhotos.findIndex((p) => p.id === id);
    if (idx >= 0) {
      urlToDelete = db.heroPhotos[idx].url;
      db.heroPhotos.splice(idx, 1);
    }
  });
